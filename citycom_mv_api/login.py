...
from citycom_mv_api.const import (LOGIN_URL,CITYCOM_MV_WEBSITE)
...
async def do_login(session: ClientSession, user_name,password :str) -> Optional[dict]:
    ...
    try:
        response=await commons.send_post_request(session, url=LOGIN_URL, data={"grant_type": "password", "username": user_name, "password": password})
...
async def manual_authorization(session: ClientSession, user_name,password :str) -> Optional[JWT]:  # pragma: no cover
    ...
    if not user_name:
        id_number = await commons.read_user_input("Enter your user: ")
    if not password:
...
async def refresh_token(session: ClientSession, token: JWT) -> Optional[JWT]:
    ...
    response = await commons.send_post_request(
        session=session, url=LOGIN_URL, data=data, headers=headers
    )
    return JWT.from_dict(response)
